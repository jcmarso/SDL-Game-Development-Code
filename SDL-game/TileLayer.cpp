...
Tileset TileLayer::getTilesetByID(int tileID) 
{
	...
	{
		if (static_cast<unsigned long long>(i) + 1 <= m_tilesets.size() - 1)
		{
			if (tileID >= m_tilesets[i].firstGridID && tileID < m_tilesets[static_cast<std::vector<Tileset, std::allocator<Tileset>>::size_type>(i) + 1].firstGridID)
			{
				return m_tilesets[i];
			}
		}
